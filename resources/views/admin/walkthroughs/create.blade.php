...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.walkthrough.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.walkthroughs.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="page">{{ trans('cruds.walkthrough.fields.page') }}</label>
                <input class="form-control {{ $errors->has('page') ? 'is-invalid' : '' }}" type="number" name="page" id="page" value="{{ old('page', '') }}" step="1">
                @if($errors->has('page'))
